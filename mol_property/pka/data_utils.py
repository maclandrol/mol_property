...
DATA_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "./train/data/pKaInWater.csv"
)
...
class DataUtils(object):
    ...
    def describe(self):
        print(
            "Unique: {} / {}".format(
                len(self.df_pka["Smiles"].unique()), self.df_pka.shape[0]
            )
        )
        print(
            "basic Unique: {} / {}".format(
                len(self.df_pka_basic["Smiles"].unique()), self.df_pka_basic.shape[0]
            )
        )
        acidic_only_cnt = len(
            set(self.df_pka_acidic["Smiles"].unique())
            - set(self.df_pka_basic["Smiles"].unique())
        )
        basic_only_cnt = len(
            set(self.df_pka_basic["Smiles"].unique())
            - set(self.df_pka_acidic["Smiles"].unique())
        ...
        print(
            "acidic_only_cnt: {}, basic_only_cnt: {}, both_pka_cnt: {}".format(
                acidic_only_cnt, basic_only_cnt, both_pka_cnt
            )
        )
    ...
    def get_classification_data(self, feature_type="morgan+macc"):
        ...
        return (
            self.get_molecular_features(
                [Chem.MolFromSmiles(mol) for mol in df_smi.index], feature_type
            ),
            df_smi["acidic"].values,
            df_smi["basic"].values,
        )
    ...
    def get_molecular_features(mols, feature_type="morgan+macc"):
        ...
        """
        fp_morgan = [
            AllChem.GetMorganFingerprintAsBitVect(mol, 2, nBits=1024) for mol in mols
        ]
        ...
        if feature_type == "morgan":
            ...
            return np.concatenate(
                [rdkit_numpy_convert(fp_morgan), rdkit_numpy_convert(fp_macc)], axis=1
            )
